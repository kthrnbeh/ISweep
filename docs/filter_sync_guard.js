...
(function () {
  ...
  async function verifyOrRepairSave() {
    ...
    // Give main.js's normal PUT /preferences request time to finish first.
  ...
  // Align historical keys immediately so the normal main.js save uses the same account.
  alignTokenKeys();
  ...
  document.addEventListener('click', (event) => {
    ...
    // Capture the current account token before main.js performs its existing save.
    alignTokenKeys();
    setStatus('Saving filters to your ISweep account…');

    // main.js first saves the current UI into isweep-settings synchronously, then
    // performs its normal PUT. Verify that result and repair only if needed.
    setTimeout(() => {
